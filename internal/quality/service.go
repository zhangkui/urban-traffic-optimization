...
func SortAcceptedInPlace(result *Result) {
	sort.Slice(result.Accepted, func(i, j int) bool { return result.Accepted[i].RecordedAt.Before(result.Accepted[j].RecordedAt) })
}
